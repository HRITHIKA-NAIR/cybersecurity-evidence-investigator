...
Use feature/investigator-v2.2 for this update stream. The three version-2 remote branches were identical before this work; master contains the older SQLite implementation. Do not merge the older implementation over the version-2 architecture without a reviewed migration.

## Local setup

Install Git, Python 3.12 or 3.13 and Node 22. Clone the repository and switch to feature/investigator-v2.2. Work on a dedicated test database. Copy .env.example into backend/.env and frontend/.env.example into frontend/.env; never commit either populated file.

From backend, create a virtual environment with python -m venv .venv. Activate it with source .venv/bin/activate on macOS/Linux or .venv\\Scripts\\Activate.ps1 in Windows PowerShell. Run python -m pip install -r requirements-dev.txt.

Use the migration-owner PostgreSQL connection only to run python scripts/init_database.py and python scripts/provision_runtime.py. The second command securely prompts for a runtime password. Then replace DATABASE_URL with the restricted runtime connection and set APP_ENV=production to avoid startup DDL when exercising that account. Development mode can initialize schema automatically only when using an authorized migration role.

Run python scripts/diagnose_database.py, then uvicorn app.main:app --host 127.0.0.1 --port 8001. In a second terminal, enter frontend, run npm ci and npm run dev. The default UI is http://localhost:5173. Configure both localhost and 127.0.0.1 only when actually used in Supabase redirect settings.

## Configuration ownership

Backend configuration includes DATABASE_URL, APP_ENV, FRONTEND_ORIGIN, SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, optional provider keys/enable flags, pool sizes and request budgets. Both the backend and browser use a publishable Supabase key; only the backend receives provider secrets and PostgreSQL credentials.

Frontend VITE_API_URL, VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY are public build values. SITE_URL, SUPPORT_EMAIL, OPERATOR_NAME and LAUNCH_READY prepare public pages after building. LAUNCH_READY=false produces noindex pages and disallows crawling. True requires usable HTTPS settings and public operator/contact details. A pre-bundle allow-list accepts only the three documented VITE variables, rejects privileged keys and origin credentials, and a post-build scan checks emitted files. No scanner is an exhaustive guarantee.

## Code workflow

Add category-specific interface behavior to InvestigationForm and category metadata, not to every result component. Keep API calls in services/api.js. Keep private data in memory; do not persist evidence in localStorage. Sign-out or account identity changes must clear results/history and invalidate in-flight responses.

Implement extraction inside parsers, deterministic rules inside detectors/services and external HTTP calls behind integrations/tools. Every new parser must bound expansion, extraction and nesting. Treat filenames, document fields, URLs and AI output as untrusted. Never render evidence with innerHTML or execute attachments.

All user data access must enter user_connection(owner_id). History summaries return only 160 content characters and selected metadata; full evidence loads on case selection. Do not obtain a raw owner connection from a request handler. New child tables need foreign keys, indexes, RLS and a policy tied to a visible owner parent. Update the schema, read/write hydration, fixtures and negative cross-user tests together. Never rely on CORS as authorization.

## Tests and release checks

Run python -m pytest -q in backend. In frontend run npm test, npm run lint, npm run build and npm run test:browser. Install its browser once with npx playwright install chromium. The browser suite uses synthetic accounts and mocked API responses, not live credentials. To run adapter integration tests, use a disposable PostgreSQL instance, set DATABASE_URL to its migration-capable account and RUN_POSTGRES_TESTS=1. The suite initializes schema and creates synthetic cases. GitHub Actions provisions PostgreSQL 16 for this purpose.

An optional embedded PostgreSQL RLS check is in scripts/test_rls_embedded.mjs. Install @electric-sql/pglite outside the production dependency tree and pass its module path to that script from backend. It validates policy behavior without a managed Supabase service; it does not replace the live adapter or deployment tests.

backend/smoke_test.py uses EVIDENCE_BASE_URL and prompts privately for a short-lived token from a disposable confirmed account. --check-isolation also prompts for a different account’s token. Obtain the token locally through the browser network tools on your own test session; never paste it in chat, a ticket or a committed file. The smoke script creates synthetic cases and deletes only the cases it created.

## Debugging and maintenance

A successful /live only proves the process runs. A failing /health points to database connectivity, schema or restricted-role readiness. Check the project state, IPv4-compatible session-pooler address, URL-encoded password, TLS and completed schema setup. Keep error logs free of raw evidence, bearer tokens and database URLs.

The default rate limiter is per process. Keep one worker for the free beta; a multi-worker or multi-replica deployment needs shared IP throttling. PostgreSQL budgets remain shared. The browser's cancellation cannot forcibly stop a running Python parser; avoid automatic retries and consider isolated jobs before larger traffic.

Keep dependencies current, review vulnerability notices, back up before schema changes and perform a restore drill. Document schema evolution rather than accumulating ad hoc production SQL. The current idempotent schema initializer is a bootstrap mechanism; ordered migrations should be introduced for breaking changes.

## Administrative deletion

Verify the requester's ownership through the published private support channel. Disable/delete the Supabase Auth identity and stop new work for that identity, let already-admitted requests complete, then remove investigations for that UUID using scripts/delete_user_data.py. Its default mode prints counts only; --execute requires typing the verified UUID. It refuses deletion while the Supabase Auth identity still exists. Remove matching usage counters and record a minimal completion audit. Explain backup/provider retention honestly. Do not ask users for credentials or publicly share their evidence.

## Owner database testing and deployment sequence
